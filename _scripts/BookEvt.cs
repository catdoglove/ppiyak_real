...
public class BookEvt : MonoBehaviour {
	...
	public GameObject Book_obj,bookInfo_obj,infoImage1_obj,infoImage2_obj;
	...
	void Start () {
		...
		if (str.Length < 3) {
			spr = Resources.LoadAll<Sprite> ("ppiyak/ppiyak_01(170x130)");
			for (int i = 0; i < 13; i++) {
				bookGM_str = "Image (" + i + ")";
				bookGM [i] = GameObject.Find (bookGM_str);
			}
		}
		for (int i = 0; i < 20; i++) {
			i++;
	...
	public void bookOpen(){
		...
		for (int i = 0; i < 6; i++) {
			...
			//GM.GetComponent<BookEvt> ().bookGM [i].GetComponent<Image> ().sprite = GM.GetComponent<BookEvt> ().spr [i];
			//}
		}
		/*
